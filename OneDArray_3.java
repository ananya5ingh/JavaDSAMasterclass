...
class Operation {
    ...
    public void insert(int location, int input) {
        ...
        try {
            if (arr1[location] != Integer.MIN_VALUE) { // O(1)
                ...
                // cause we're doing insertion here, that would be upd ation
            } else {
    ...
    public void deleteElement(int indexOfElement) {
        ...
        try {
            ...
            arr1[indexOfElement] = Integer.MIN_VALUE; // TC: O(1)
            System.out.println("Element has been deleted successfully."); // TC: O(1)
        } catch (ArrayIndexOutOfBoundsException e) {

            System.out.println("This array index is out of bounds."); // TC: O(1)
        }
    }
    // Therefore for this deletion method TC: O(1)
    // SC: O(1), since no extra memory is required to perform operation
}
